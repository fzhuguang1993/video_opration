...
class AssetsPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.update_time_label.setObjectName("stats_label")
        ...
        self.tab_widget.addTab(self._create_empty_tab(), "请先登录")
    ...
    # 数据加载
    # ================================================================
    def _load_data(self, sort_col: int = -1, sort_order: str = "ASC"):
        try:
            ...
            order_clause = ""
            if sort_col >= 0 and sort_col in self.column_map:
                db_field = self.column_map[sort_col]
                if db_field == "row_num":
                    db_field = "vt.trace_code"
                order_clause = f"ORDER BY {db_field} {sort_order}"
            else:
                order_clause = "ORDER BY vt.record_date DESC"
            ...
            self._rebuild_tabs()
            self._update_last_update_time()
            self.logger.info(f"加载完成: {len(self.all_data)} 条视频, 角色={self.user_role}")
    ...
    def _rebuild_tabs(self):
        ...
        if self._is_admin():
            ...
            my_tab = self._create_my_tab()
            self.tab_widget.addTab(my_tab, f"我的数据 ({len(self.all_data)})")
    ...
    # 我的数据 Tab（非超管）
    # ================================================================
    def _create_my_tab(self) -> QWidget:
        widget = QWidget()
        layout = QVBoxLayout(widget)
        layout.setSpacing(12)
    ...
    def _create_data_table(self) -> QTableWidget:
        ...
        widths = [50, 100, 180, 100, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80]
    ...
    def _fill_table(self, table: QTableWidget, data: list):
        ...
        for row, record in enumerate(data):
            ...
            click_1k_cost = record[18] if record[18] is not None else 0
            ...
            values = [
                ...
                f"{daily_cost:.2f}" if daily_cost else "0.00",
                f"{week_cost:.2f}" if week_cost else "0.00",
                str(impression),
                str(clicks),
                f"{click_rate:.2f}%",
                str(conversion),
                f"{conversion_rate:.2f}%",
                f"{click_1k_cost:.2f}" if click_1k_cost else "0.00",
                f"{three_sec:.2f}%",
                f"{five_sec:.2f}%",
                rating,
            ]
    ...
    def _on_header_clicked(self, col: int):
        ...
        self._load_data(sort_col=col, sort_order=self.sort_order)
    ...
    def _auto_refresh(self):
        self.logger.info("🔄 自动刷新资产管理数据")
        self._load_data()

    def _update_last_update_time(self):
        self.last_update_time = datetime.now()
        time_str = self.last_update_time.strftime('%Y-%m-%d %H:%M')
    ...
    def _rebuild_tabs_filtered(self):
        ...
        self.tab_widget.addTab(overview_tab, "搜索结果")
    ...
    def _on_sync_kuaishou(self):
        ...
        try:
            ...
            progress = QProgressDialog("正在同步快手数据...", "取消", 0, 0, self)
            progress.setWindowModality(Qt.WindowModal)
            progress.show()
            QApplication.processEvents()

            result = sync_kuaishou_data()
            progress.close()
            ...
            QMessageBox.information(
                ...
                f"  📝 日报表更新: {result.get('report_updated', 0)}"
            )
